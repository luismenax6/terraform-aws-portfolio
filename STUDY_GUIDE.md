have# Terraform AWS Portfolio — Study Guide
...
|---------|-------|-------|-------|
| P0 | Static Website — S3 + CloudFront + Route53 | Foundations | Week 1-3 |
| P1 | Terraform Fundamentals | Foundations | Week 4-5 |
| P2 | VPC + EC2 — Variables & Data Sources | Core Skills | Week 6-7 |
| P3 | App 3-Tier — Modules + Remote State | Core Skills | Week 8-9 |
| P4 | ECS Fargate — State Management + Workspaces | Advanced | Week 10-11 |
| P5 | Landing Zone — Multi-account + HCP Terraform | Advanced | Week 12 |

---

## P0 — Static Website: S3 + CloudFront + Route53
**Phase:** Foundations | **Week 1-3**

> Personal portfolio deployed on AWS. S3 for static hosting, CloudFront as CDN with HTTPS, ACM certificate with DNS validation, Route53 alias records, and GitHub Actions CI/CD with OIDC.

---

### Step 1 — Bootstrap: State Backend

> Before any project create the S3 bucket that stores Terraform state. Done manually once, shared across all projects. Since Terraform 1.10, locking is native in S3 using a `.tflock` file — no DynamoDB needed.

**What to do:**
- Create S3 bucket `luismena-terraform-state` in `us-east-1`
- Enable versioning on the bucket
- Enable SSE-S3 encryption and block all public access
- Verify the bucket exists before moving on

**Tips:**
- S3 → Create bucket → keep "Block all public access" enabled — state should never be public
- Versioning is under Properties → Bucket Versioning → Enable. Lets you recover a previous state if something goes wrong
- Native locking creates a `.tflock` file in the same bucket during active operations — no extra cost
- Verify with: `aws s3 ls | grep terraform-state`
- Make sure you have Terraform >= 1.10: `terraform version`

> **Exam note:** This bucket is reused in P2, P3, P4 and P5 — each project uses a different key inside the same bucket.

---

### Step 2 — GitHub Repo + Folder Structure

> Each project lives in its own repo. The portfolio repo includes your HTML/CSS/JS files alongside the Terraform code.

**What to do:**
- Create repo on GitHub (public — it's your portfolio)
- Create the `website/` folder with your portfolio files
- Create `.github/workflows/` for the pipeline
- Create `.gitignore` excluding Terraform files and `terraform.tfvars`
- Root files: `providers.tf`, `variables.tf`, `outputs.tf`, `main.tf`, `acm.tf`, `route53.tf`, `oidc.tf`, `terraform.tfvars`

**Tips:**
- When creating the repo on GitHub select Add .gitignore → Terraform — GitHub has an official template
- To track empty folders in git: create a `.gitkeep` inside until you have the yml file
- `git init → git remote add origin URL → git add . → git commit → git push -u origin main`

> **Exam note:** `terraform.tfvars` must NEVER go to the repo. Pipelines use GitHub Actions Variables instead.

---

### Step 3 — providers.tf: Provider and S3 Backend

> The S3 backend connects your local config and the pipeline to the same remote state. The `us_east_1` provider alias is required because CloudFront demands the ACM certificate be in `us-east-1`.

**What to do:**
- Declare `required_providers` with `hashicorp/aws ~> 5.0`
- Configure the `backend "s3"` block with `bucket`, `key`, `region`, `encrypt=true`, `use_lockfile=true`
- Declare main `aws` provider with region from a variable
- Declare a second `aws` provider with `alias = "us_east_1"` pointing to `us-east-1`
- Run `terraform init` — it should connect to the S3 bucket

**Tips:**
- The `terraform {}` block contains `required_providers {}` and `backend {}` — they are not separate top-level blocks
- The aliased provider goes **after** the `terraform {}` block, not inside it
- `terraform init` with a remote backend asks if you want to copy local state to remote — say yes
- If init fails due to credentials: `aws sts get-caller-identity`
- After init, `.terraform.lock.hcl` is created with exact provider versions

> **Exam note:** The backend block goes inside `terraform {}`, not in `provider {}`. After adding or changing the backend always run `terraform init`.

---

### Step 4 — variables.tf + terraform.tfvars

> Variables make the config reusable. The validation block lets you reject invalid values before Terraform tries to create any resource.

**What to do:**
- `aws_region`: string, default `us-east-1`
- `domain_name`: string, default `luismena.info`
- `bucket_name`: string, no default (required)
- `environment`: string, default `prod`, with validation accepting only `dev/staging/prod`
- `tags`: `map(string)` with defaults for Project, ManagedBy, Owner
- Create `terraform.tfvars` with values for `bucket_name` and `environment`

**Tips:**
- A validation block has two required attributes: `condition` (bool expression) and `error_message` (string)
- Enum validation: `condition = contains(["dev","staging","prod"], var.environment)`
- In `terraform.tfvars` the syntax is: `bucket_name = "value"` — no `variable {}` block, just direct assignment
- Run `terraform validate` after creating `variables.tf` to catch syntax errors

> **Exam note:** Variable precedence (lowest to highest): default → tfvars → auto.tfvars → `-var-file` → `-var` → `TF_VAR_*`

---

### Step 5 — acm.tf: SSL Certificate with DNS Validation

> ACM issues free certificates. Since the `luismena.info` Hosted Zone already exists in Route53, Terraform can create the certificate and DNS validation records fully automatically.

**What to do:**
- Data source `aws_route53_zone` reading `luismena.info` with `private_zone = false`
- `aws_acm_certificate` for `luismena.info` and `www.luismena.info`, DNS method, using `provider = aws.us_east_1`
- `aws_route53_record` for validation using `for_each` over `domain_validation_options`
- `aws_acm_certificate_validation` waiting for CNAME propagation
- CloudFront uses the ARN from `aws_acm_certificate_validation`, not from the certificate directly

**Tips:**
- The data source is declared without a provider alias — it reads the zone regardless of region
- `for_each` pattern: `for_each = { for dvo in aws_acm_certificate.this.domain_validation_options : dvo.domain_name => dvo }`
- `aws_acm_certificate_validation` can take several minutes — it's normal for `terraform apply` to wait there

> **Exam note:** `depends_on` is for dependencies Terraform cannot detect automatically. A direct reference already creates an implicit dependency — no `depends_on` needed here.

---

### Step 6 — main.tf: S3 + CloudFront

> Traffic flow: user → Route53 → CloudFront → S3. S3 has public access blocked — only CloudFront can read files via Origin Access Control (OAC).

**What to do:**
- `aws_s3_bucket` with bucket name from variable
- `aws_s3_bucket_website_configuration` with `index_document` and `error_document`
- `aws_s3_bucket_public_access_block` with all blocks set to `true`
- `aws_cloudfront_origin_access_control` with `signing_behavior = "always"` and `signing_protocol = "sigv4"`
- `aws_cloudfront_distribution` with aliases, origin, viewer_certificate, HTTP→HTTPS redirect
- `aws_s3_bucket_policy` using `jsonencode()` allowing only `GetObject` from the OAC

**Tips:**
- `aws_s3_bucket_website_configuration` is a separate resource — reference with `bucket = aws_s3_bucket.this.id`
- Use `bucket_regional_domain_name` as the origin, not `website_endpoint`
- `viewer_certificate` needs `acm_certificate_arn` (from the validation resource), `minimum_protocol_version`, and `ssl_support_method = "sni-only"`
- Bucket policy condition: `StringEquals: aws:SourceArn = CloudFront distribution ARN`
- Add `depends_on = [aws_s3_bucket_public_access_block.this]` to the bucket policy

> **Exam note:** Implicit dependencies are created by referencing a resource. Terraform resolves creation order automatically without `depends_on`.

---

### Step 7 — route53.tf + outputs.tf

> To point `luismena.info` to CloudFront use Alias records, not CNAME — CNAMEs don't work at the domain apex.

**What to do:**
- `aws_route53_record` type A with alias for `luismena.info` pointing to CloudFront
- `aws_route53_record` type A with alias for `www.luismena.info`
- `outputs.tf` with `website_url`, `cloudfront_domain`, `cloudfront_distribution_id`, `s3_bucket_name`
- Run `terraform plan → apply`
- Verify `https://luismena.info` loads

**Tips:**
- Use `aws_cloudfront_distribution.this.hosted_zone_id` — Terraform resolves it automatically (same as the fixed `Z2FDTNDATAQYW2`)
- For www: create a second record pointing to the same CloudFront
- Verify with: `dig luismena.info` — should return CloudFront IPs
- All outputs must have a `description`

> **Exam note:** Alias has no query cost, supports the domain apex, and points to AWS resources. CNAME doesn't work at the apex.

---

### Step 8 — oidc.tf: IAM Role for GitHub Actions

> OIDC eliminates the need to store AWS Access Keys in GitHub. GitHub generates a JWT per execution, AWS validates it and returns temporary credentials that expire in 1 hour.

**What to do:**
- `aws_iam_openid_connect_provider` for `token.actions.githubusercontent.com`
- `aws_iam_role` with trust policy allowing `AssumeRoleWithWebIdentity` from GitHub
- Condition `StringLike`: `sub` must match `repo:luismenax6/*:*`
- Condition `StringEquals`: `aud` must be `sts.amazonaws.com`
- `aws_iam_role_policy_attachment` with `AdministratorAccess`
- Output the role ARN

**Tips:**
- GitHub OIDC thumbprint: `6938fd4d98bab03faadb97b34396831e3780aea1` — fixed, no need to calculate
- Use `jsonencode()` for the trust policy inside `assume_role_policy`
- `AdministratorAccess` is for labs only — in production scope down permissions per project
- After apply, copy the role ARN from `terraform output` — needed as a GitHub Actions Variable

> **Exam note:** `repo:luismenax6/*:*` allows all your repos. For tighter security use the specific repo and branch.

---

### Step 9 — GitHub Actions: Full Pipeline

> The pipeline applies Terraform on push to main and syncs website files to S3 with CloudFront cache invalidation.

**What to do:**
- Create `.github/workflows/deploy.yml` with trigger on push to `main`
- Permissions: `id-token: write`, `contents: read`
- Job `terraform`: checkout → configure-aws-credentials → setup-terraform → init → fmt --check → validate → apply
- Job `deploy-website`: build Astro site → sync `./website/dist` to S3 → CloudFront invalidation
- Add GitHub Variables: `AWS_ROLE_ARN`, `AWS_REGION`, `BUCKET_NAME`, `CF_DISTRIBUTION_ID`

**Tips:**
- `configure-aws-credentials` uses `role-to-assume` — no `AWS_ACCESS_KEY_ID` or `AWS_SECRET_ACCESS_KEY` needed
- `terraform fmt --check` fails if code is not formatted — run `terraform fmt` locally before every push
- `aws s3 sync ./website/dist s3://BUCKET --delete` — `--delete` removes files no longer in the repo
- `CF_DISTRIBUTION_ID` is not sensitive — store as a GitHub Variable, not a Secret

---

## P1 — Terraform Fundamentals
**Phase:** Foundations | **Week 4-5**
